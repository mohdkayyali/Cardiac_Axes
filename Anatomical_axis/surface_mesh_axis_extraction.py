...
import shutil
import pandas as pd
from mesh_processing import import_mesh_2, axis_by_PCA,axis_by_labels, spherecity_index

source_dir = '/netapp/cme_digital_twins/UKBB_88878/Data/'  

f = os.listdir('/netapp/cme_digital_twins/UKBB_Raw_88878/Data/')

files = os.listdir(source_dir)

# Filter the first 100 files that end with '.xml'
Mesh_files = [file for file in files if file.startswith('UKBB')]

# total_size = sum(os.path.getsize(os.path.join(source_dir, file)) for file in Mesh_files)
result_df = pd.DataFrame()
p=0
for patient_file in Mesh_files:
    patient_id = patient_file.split('_')[2]
    source_file_path = os.path.join(source_dir, patient_file)
    contents = os.listdir(source_file_path)
    # print(contents)
    # if len(contents) == 2:
    #     if contents == ['Instance_2','Instance_3']:
    #         p = p+1
    #         print(patient_id)
            
    path = os.path.join(source_file_path, contents[0])
    contents2 = os.listdir(path)
    
    
    try:
        
        patient_folder = os.path.join(path,'Mesh_Outputs')
        mesh_dict = import_mesh_2(patient_folder)
        ...
        base_key_1 = ['MV']  # Specify the mesh parts you want to process
        base_key_2 = ['MV','AV']  # Specify the mesh parts you want to process
        base_key_3 = ['MV','AV','TV','PV']  # Specify the mesh parts you want to process
        apex_type = 'endo'  # Specify the apex type
        ...
        result_df = result_df._append({'eid': patient_id,
...
result_df.to_csv('anatomical_axis_results_full.csv', index=False)  # Save without row indices

#%% Extracting ES & ED (UPDATED)
import pandas as pd
